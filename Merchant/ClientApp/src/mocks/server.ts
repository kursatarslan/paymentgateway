...
// Activity
mock.onGet("/api/activitylogs").reply(200, activityLogs);

// Auth
mock.onPut("/api/password").reply(({ data }) => [200, data]);
mock.onPost("/api/forgotpassword").reply(200);
mock.onPost("/api/forgotpasswordsubmit").reply(200);
mock.onPost("/api/auth/login").reply(200, {"accessToken": "ACCESSTOKEN123", "refreshToken": "ACCESSTOKEN123", status: "success", userName: "kursat.arslan"});
mock.onPost("/api/logout").reply(200);
mock.onPost("/api/register").reply(201);
mock
  .onGet("/api/userinfo", { params: { username: "kursat.arslan" } })
  .reply(200, userInfo);
...
// Profile
mock.onGet("/api/profileinfo").reply(200, profileInfo);
mock.onPut("/api/profileinfo").reply(({ data }) => [200, data]);
